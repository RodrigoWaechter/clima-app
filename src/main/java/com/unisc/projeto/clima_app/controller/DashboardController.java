...
public class DashboardController {
	...
	//quando abre o programa ja carrega uma cidade
	...
	public void actionAtualizarDados() {
		if (localizacaoAtual != null) {
			...
			JOptionPane.showMessageDialog(view, "Nenhuma localização selecionada para atualizar.", "Aviso",
	...
	public void actionBuscarLocalizacao() {
		...
		if (nomeCidade != null && !nomeCidade.trim().isEmpty()) {
			...
			JOptionPane.showMessageDialog(view, "Por favor, digite o nome de uma cidade.", "Campo Vazio",
	...
	private void actionBuscarDadosPorLocalizacao(String nomeCidade) {
		...
		executor.submit(() -> {
			try {
				...
				SwingUtilities.invokeLater(() -> loadingState(false));
			}
		});
	}
	
	// so firula pra alterar o cursor e desabilitar os campos enquanto pesquisa....
	private void loadingState(Boolean loading) {
	...
	private void atualizarTabelaPrevisao(List<DadoDiario> previsaoDiaria) {
		...
		if (previsaoDiaria == null)
			return;
		DateTimeFormatter dayFormatter = DateTimeFormatter.ofPattern("E, dd/MM", new Locale("pt", "BR"));

		for (DadoDiario dia : previsaoDiaria) {
			ImageIcon icon = IconUtils.carregarIconeRedimensionado(WeatherCodeUtil.getIcon(dia.getCdClima()), 24, 24);
			String description = WeatherCodeUtil.getWeatherDescription(dia.getCdClima());
		
			Object[] rowData = { 
					dia.getData().format(dayFormatter),
					new Object[] { icon, description },
	...
	private void atualizarCondicoesAtuais(DadoHorario dadoAtual) {
		...
		view.getLblIconeCondicaoAtual().setIcon(IconUtils.carregarIconeRedimensionado(WeatherCodeUtil.getIcon(weatherCode), 80, 80));
	...
	private void atualizarPrevisaoHoraria(List<DadoHorario> previsaoHoraria) {
		...
		for (DadoHorario hora : previsaoHoraria) {
			...
			String iconPath = WeatherCodeUtil.getIcon(hora.getCdClima());
